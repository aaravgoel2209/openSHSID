...
def _set_sqlite_pragmas(sender, connection, **kwargs):
    ...
    参数在 config.json 的 django.sqlite_pragmas 中维护。"""
    if connection.vendor == 'sqlite':
        p = cfg['django']['sqlite_pragmas']
        cursor = connection.cursor()
        cursor.execute(f'PRAGMA journal_mode={p["journal_mode"]};')
        cursor.execute(f'PRAGMA synchronous={p["synchronous"]};')
        cursor.execute(f'PRAGMA busy_timeout={p["busy_timeout"]};')
